...
export function RequestDetails() {
  ...
  const [confirmedTypes, setConfirmedTypes] = useState<Record<number, string>>(
    () => {
      ...
      return Object.fromEntries(
        request.items.map((item) => [item.id, item.accessType])
      )
    }
  )
  ...
  const handleActionSubmit = () => {
    ...
    if (actionType === "REJECT") {
      actionableItems.forEach((item) => {
        rejectItem(request.id, item.id, comment.trim())
      })
    }
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="space-y-3">
            ...
            <p className="text-sm text-muted-foreground">
              Requested on {formatDate(request.requestedAt)} by{" "}
            ...
            <div className="space-y-3">
              {request.items.map((item) => (
                ...
                >
                  ...
                  {getDaysUntilExpiry(item) < 30 &&
                    getDaysUntilExpiry(item) > 0 && (
      ...
      <Dialog
        open={isDialogOpen}
        onOpenChange={(value) =>
          value ? setIsDialogOpen(true) : closeActionDialog()
        }
      >
              ...
              <div className="space-y-3">
                {request.items.map((item) => (
                  ...
                  >
                    <div className="grid gap-4 sm:grid-cols-2">
                      <div>
                        <Label htmlFor={`dialog-item-system-${item.id}`}>
                          System
                        </Label>
                        <Input
                          id={`dialog-item-system-${item.id}`}
                          value={item.system}
                          disabled
                        />
                      </div>
                      <div>
                        <Label htmlFor={`dialog-item-access-${item.id}`}>
                          Access Type
                        </Label>
                        <Select
