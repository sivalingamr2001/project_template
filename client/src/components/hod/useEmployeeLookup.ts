...
export const useEmployeeLookup = (empId: number) => {
  ...
  const accesses = employeeRequests.flatMap((request) =>
    request.items.length === 0
      ...
      : request.items.map((item) => ({
          ...
          requestedAt: request.requestedAt,
          approvedBy:
            item.approvalHistory.find((entry) => entry.action === "HODApproved")?.approverName,
          itApprovedBy:
            item.approvalHistory.find((entry) => entry.action === "ITApproved")?.approverName,
        }))
  )
  ...
  return {
    data: {
      ...
      pending: accesses.filter((item) =>
        ["PendingHOD", "PendingIT"].includes(item.status)
      ).length,
