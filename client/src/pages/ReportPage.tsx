...
import type { ReportPeriod, BudgetRecord } from "@/types/budget"
import {
  useProjectBudget,
  useProjectBudgets,
  useMonthlyTrend,
...
function calculateSummary(budgets: BudgetRecord[]) {
  const planned = budgets
    .flatMap((budget) => budget.lineItems)
    .reduce((sum, item) => sum + item.plannedAmount, 0)
  const actual = budgets
    .flatMap((budget) => budget.lineItems)
  ...
  return {
    ...
    variancePct,
    utilisationPct,
    activeProjects: budgets.filter((budget) => budget.status === "active")
      .length,
  }
}
...
export default function ReportPage() {
  ...
  const selectedBudget =
    projectQuery !== "all" ? useProjectBudget(projectQuery) : undefined
  const filteredBudgets = selectedBudget ? [selectedBudget] : budgets

  const summary = useMemo(
    () => calculateSummary(filteredBudgets),
    [filteredBudgets]
  )
  ...
  const breakdown = useMemo<BreakdownGroup[]>(() => {
      ...
      .forEach((item) => {
        ...
        if (existing) {
          rows.set(key, {
            ...
            utilisationPct:
              existing.planned + planned
                ? ((existing.actual + actual) / (existing.planned + planned)) *
                  100
                : 0,
          })
        } else {
    ...
    const result = Array.from(grouped.values()).map((group) => ({
      ...group,
      subtotalPct: group.subtotalPlanned
        ? (group.subtotalVariance / group.subtotalPlanned) * 100
        : 0,
    }))
  ...
  const phaseStats = useMemo(
    () =>
      reportPhases.map((phase) => {
        const phaseBudgets = filteredBudgets.filter(
          (budget) => budget.phase === phase
        )
        const planned = phaseBudgets
          .flatMap((budget) => budget.lineItems)
          .reduce((sum, item) => sum + item.plannedAmount, 0)
        const actual = phaseBudgets
          .flatMap((budget) => budget.lineItems)
          .reduce((sum, item) => sum + item.actualAmount, 0)
        const utilisationPct = planned ? (actual / planned) * 100 : 0
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* Header Section */}
      <div className="flex flex-col gap-4 rounded-3xl border border-border bg-card p-6 shadow-sm transition-colors md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          />
          <Button variant="outline">Export PDF / Excel</Button>
        </div>
      </div>

      {/* Summary Cards */}
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {/* Reusable style: text-foreground for titles, text-muted-foreground for labels */}
        <Card className="border border-border bg-card shadow-none">
          <CardContent className="space-y-1 px-4 py-5">
            ...
            <div className="text-sm font-medium text-muted-foreground/60">
              Filtered by period and project
            </div>
          </CardContent>
        </Card>
      ...
      <Card className="border border-border bg-card p-4">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <p className="text-sm font-medium text-foreground">Report window</p>
            <p className="text-xs text-muted-foreground">
              Mode: {periodQuery.charAt(0).toUpperCase() + periodQuery.slice(1)}
            </p>
          </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <BudgetVarianceChart
          data={trendData.map((point) => ({
            ...
            actual: point.actual,
            variancePct: point.planned
              ? ((point.planned - point.actual) / point.planned) * 100
              : 0,
          }))}
        />
        <Card className="border border-border bg-card p-4">
          ...
          <CardContent className="space-y-3 px-0 pt-0 pb-0">
            <div className="grid gap-3 sm:grid-cols-2">
              {/* Use secondary or muted bg instead of slate-950 */}
              ...
              <div className="rounded-3xl border border-border bg-secondary/50 p-4">
                ...
                <div
                  className={`mt-2 text-xl font-semibold ${summary.variance >= 0 ? "text-emerald-500" : "text-destructive"}`}
                >
                  {formatCurrency(summary.variance)}
                </div>
              </div>
              <div className="rounded-3xl border border-border bg-secondary/50 p-4">
                ...
                <div className="mt-2 text-xl font-semibold text-foreground">
                  {formatPercent(summary.utilisationPct)}
                </div>
              </div>
            </div>
          </CardContent>
