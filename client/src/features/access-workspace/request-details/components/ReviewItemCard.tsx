  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4">
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Field label="Confirm Access Type By HOD">
          {isHod ? (
            <Select
              value={String(value)}
              onValueChange={(next) => onConfirmChange(Number(next))}
            >
      ...
      <Field className="mt-4" label="Reason">
        <Textarea disabled rows={2} value={item.reason} />
      </Field>
    </div>
  )
}
