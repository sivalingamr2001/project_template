...
function AccessValidityBadge({ itemId }: { itemId?: number }) {
    const [expiryData, setExpiryData] = useState<{ approvedOn: string; expiresOn: string } | null>(null)
            ...
            <div className="flex flex-col items-center gap-1">
                <div className="flex items-center gap-3 px-4 py-1.5">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-[11px] font-semibold uppercase text-destructive/80 tracking-tight">
                            Expired On
                        </span>
                    </div>

                    {/* Always use text-destructive for the timer text */}
                    <div className="font-mono text-sm font-bold min-w-[125px] text-destructive">
                        {timeLeft}
                    </div>
                </div>
            </div>
