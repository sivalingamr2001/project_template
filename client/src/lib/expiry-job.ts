...
): Array<{ requestId: string; requestName: string; item: any }> {
  const expiringItems: Array<{ requestId: string; requestName: string; item: any }> = [];
...
export function getItemsExpiringToday(requests: AccessRequest[]): number {
  ...
  return requests.reduce((count, request) => {
    return count + request.items.filter(item => 
      item.status === 'ACTIVE' && 
 ...
 */
export function revokeAccessItem(request: AccessRequest, itemId: string): AccessRequest {
  return {
    ...request,
