    ...
    <TooltipProvider delayDuration={0}>
      <aside
        className={cn(
          ...
          !isCollapsed && "border-r lg:border-r lg:border-border",
          variant === "desktop"
          ...
          {filteredItems.map((item) => {
            ...
            return (
              <div key={item.id} className="w-full">
                ...
                {hasChildren && isOpen && (
                  <div className="mt-1 ml-4 animate-in space-y-1 border-l border-border/50 pl-4 duration-200 slide-in-from-top-1">
                    {item.children?.map((child) => (
                      <Link
                        key={child.path}
                        to={child.path}
