...
const SearchInput: React.FC<SearchInputProps> = ({ value, onChange, placeholder = "Search all columns…" }) => {
  const id = useId();
  return (
    <div
      role="search"
    ...
    >
      ...
      <span style={{ position: "absolute", left: 9, top: "50%", transform: "translateY(-50%)", color: "var(--color-text-secondary)", pointerEvents: "none" }}>
        <SearchIcon />
      </span>
      <input
        id={id}
        ...
        >
          ...
          {showClearFilters && (
            ...
            >
              ...
              {activeFiltersCount > 0 && <Badge count={activeFiltersCount} color="#e67700" />}
            </ToolBtn>
          )}

          {showExportCsv && (
            <ToolBtn variant="default" onClick={onExportCsv} title="Export visible data as CSV">
              <DownloadIcon />
